  ...
  if (status === "done") {
    return (
      <div className="bg-card border border-line rounded-2xl p-[18px]">
        <p className="text-sm">You're subscribed! You'll get an email when new updates are posted.</p>
      </div>
    );
  }
    ...
    <div className="w-full mx-auto px-4 py-4">
      <div className="flex flex-col lg:flex-row gap-3.5 items-start">
        <div className="flex-1 w-full min-w-0 grid gap-3.5">
          ...
          {updates.length === 0 ? (
            <div className="bg-card border border-line rounded-2xl p-[18px]">
              <div className="text-muted">No trail updates yet. Check back soon!</div>
            </div>
          ) : (
            updates.map((u) => (
              <div
